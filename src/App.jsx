    ...
    export default function App() {
        ...
        const requestSort = (key) => {
            ...
            if (sortConfig.key === key && sortConfig.direction === 'ascending') {
                ...
                // Optional: cycle back to default or remove sort
                // direction = 'ascending'; // Cycle back to ascending
                 setSortConfig({ key: 'order', direction: 'ascending' }); // Go back to default order
                 return;
            }
        ...
        }, [filteredAndSortedIssues, isDesktop]); // Re-check if isDesktop changes
        ...
        const handleColumnDragSort = () => {
             ...
             const draggedColId = visibleCols[dragIndexInVisible]?.id;
             const dropColId = visibleCols[dropIndexInVisible]?.id;

             if (!draggedColId || !dropColId) return; // Safety check
             ...
             if (originalDragIndex === -1 || originalDropIndex === -1) return; // Safety check

             const newColumnsOrder = [...columns];
             const [draggedItem] = newColumnsOrder.splice(originalDragIndex, 1);
             newColumnsOrder.splice(originalDropIndex, 0, draggedItem);
        ...
        const handleDragEnd = async () => {
            ...
            // Create a map of the new visual order based on IDs
            ...
            const fullyReorderedIssues = [...issues].sort((a, b) => {
                const orderA = newOrderMap.has(a.id) ? newOrderMap.get(a.id) : Infinity; // Place items not in current view at the end
                const orderB = newOrderMap.has(b.id) ? newOrderMap.get(b.id) : Infinity;
                
                // If both are outside the current view, maintain original relative order (e.g., based on existing order or createdAt)
                 if (orderA === Infinity && orderB === Infinity) {
                    return (a.order || 0) - (b.order || 0); // Fallback to existing order
                }
            ...
            const updatePromises = fullyReorderedIssues.map((issue, index) => {
                ...
                // Use index as the new 'order'. Consider timestamp for robustness if needed.
            ...
            try {
                await Promise.all(updatePromises);
                 ...
                 // No need for optimistic update if Firestore listener is fast enough.
                 // setIssues(reorderedIssuesForDisplay); // Optional: Optimistic update
            } catch (error) {
                console.error("Error reordering issues in Firestore:", error);
                 // Revert optimistic update if it failed
                // setIssues(issues); // Revert back to original fetched order
                ...
                <div className="flex flex-1 min-h-0">
                    <main className={`flex-col min-w-0 ${isDesktop || !selectedIssue ? 'flex' : 'hidden'}`} style={{ width: isDesktop && isDetailPanelVisible ? `${panelWidth}px` : '100%' }}>
                                ...
                                <thead className="bg-gray-50 sticky top-0 z-10">
                                    <tr>
                                        {columns.filter(c => c.isVisible).map((col, i) => (
                                            ...
                                            >
                                                <div 
                                                    draggable={col.id !== 'deviationNo'} // Only allow drag on non-# columns for headers
                                                    ...
                                                    className={`flex items-center ${col.id !== 'deviationNo' ? 'cursor-move' : ''}`}
                                                    onClick={() => col.id === 'deviationNo' && requestSort('deviationNo')} // Only add click sort for '#'
                                                >
                                                    <span>{col.label}</span>
                                                     {col.id === 'deviationNo' && (
                                                        <div className="ml-1 flex flex-col cursor-pointer">
                        ...
                        <div className="block md:hidden flex-grow overflow-y-auto p-2 space-y-2 bg-gray-100">
                             {loading ? <p className="text-center py-10">Loading issues...</p> : filteredAndSortedIssues.map((issue, index) => (
                                <div key={issue.id} 
